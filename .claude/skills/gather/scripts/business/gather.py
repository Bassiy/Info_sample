"""
ビジネス・業界動向を収集して一つのレポートにまとめるスクリプト

収集方針:
  Step1: NHK RSS（経済カテゴリ）で「今日何が重要か」を把握する
  Step2: 各NHK記事タイトルをキーワードにGoogle News RSSを検索し、
         複数媒体（日経・ロイター・Bloomberg・Business Insiderなど）の見出しを取得して description を補完する
  Step3: ITmedia・Business Insider Japan からも収集し、NHK未収録の話題を補う
...
NHK_RSS = "https://www.nhk.or.jp/rss/news/cat7.xml"   # 経済カテゴリ
...
def save_report(nhk_items, supplemental, date_str):
    output_dir = os.path.expanduser("~/Documents/info/sources/business")
